...
<table class="table">
  ...
  <tr><th>Storage</th><td><code><?= e($report['storage']) ?></code> — <?= $report['writable'] ? 'writable' : '<strong>not writable</strong>' ?></td></tr>
  ...
  <tr><th>Outbound HTTPS</th><td><?= $report['outbound']['ok'] ? 'works' : '<strong>blocked</strong>' ?> — <?= e($report['outbound']['message']) ?> <a href="?probe=1">Check again</a></td></tr>
</table>
<h2>Limits</h2>
<table class="table">
...
<?php foreach ($report['extensions'] as $ext => $info): ?>
  <tr><th><code><?= e($ext) ?></code></th><td><?= $info['loaded'] ? 'loaded' : ($info['required'] ? '<strong>missing — required</strong>' : 'missing (optional)') ?></td></tr>
<?php endforeach ?>
</table>
